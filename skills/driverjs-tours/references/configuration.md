...
Exact fields, types, and defaults for every configuration object. Source: driver.js official docs, verified against **v1.8.0**. Config, steps, and state belong to the specific `driver()` instance you set them on — instances never share state (fixed in 1.7.0; earlier versions leaked config between instances).
