...
Current release: **1.8.0**. Everything below reflects the 1.x API.
...
The tags above mirror the upstream docs and are **unpinned and unverified — do not ship them as-is.** For production, prefer `npm install` (bundled, versioned). If you must use the CDN, pin an exact version instead of `@latest` and add Subresource Integrity to *every* tag you include, the hints bundle included:
...
`@latest` silently pulls new code on every page load, so an upstream or CDN compromise lands directly in your app and SRI cannot protect a URL whose contents are expected to change. Grab the current hash from the jsDelivr "SRI" panel for each exact file.
